...
public class PageInitializer {
	 ...
	 *
	 * @param container
	 *            FluentPage or FluentAdapter object.
	 *
	...
	private void injectPageIntoContainer(Fluent container) throws ClassNotFoundException, IllegalAccessException {
		for (Class cls = container.getClass(); FluentAdapter.class.isAssignableFrom(cls)
				|| FluentPage.class.isAssignableFrom(cls); cls = cls.getSuperclass()) {
			...
			for (Field fieldFromPage : classz.getDeclaredFields()) {
				if (isFluentWebElementField(fieldFromPage)) {
					...
					if (elem == null) {
						...
						proxyElement(new AjaxElementLocatorFactory(this.fluent.getDriver(), elem.timeOutInSeconds()),
								page, fieldFromPage);
					}
				}
			}
	...
	private boolean isFluentWebElementField(Field field) {
		try {
			return !Modifier.isFinal(field.getModifiers())
					&& (isFluentList(field) || field.getType().getConstructor(WebElement.class) != null);
		} catch (Exception e) {
	...
	private <T extends FluentPage> T constructPageWithParams(Class<T> cls, Object[] params)
		...
		try {
			...
			if (params.length != 0) {
				throw new PageInitializerException("You provided the wrong arguments to the createPage method, "
						+ "if you just want to use a page with a default constructor, use @Page or createPage("
						+ cls.getSimpleName() + ".class)", ex);
			} else {
				throw ex;
			}
		}
	...
	private <T extends FluentPage> void initBaseUrl(T page, Class<?> parent)
	...
	private <T extends FluentPage> void initDriver(T page, Class<?> parent)
			throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
	...
	private static void proxyElement(ElementLocatorFactory factory, Object page, Field field) {
		...
		try {
			...
			if (isFluentList(field)) {
				// LocatingElementListHandler is not a good choice, elements are
				// FluentWebElement not WebElement.
				final InvocationHandler handler = new FluentListInvocationHandler(locator);
				FluentList<FluentWebElement> proxy = (FluentList<FluentWebElement>) Proxy
						.newProxyInstance(page.getClass().getClassLoader(), new Class[] { FluentList.class }, handler);
	...
	private static class FluentListInvocationHandler implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			FluentListImpl list = new FluentListImpl(
					FluentIterable.from(elements).transform(new Function<WebElement, FluentWebElement>() {
						@Override
